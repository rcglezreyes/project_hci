...
const RouteProvider = ({ children }) => {
  ...
  useEffect(() => {
    const mapRoutesKeys = [
      {
        subtext: 'item',
        keys: ['itemPage', 'itemRowsPerPage'],
      },
      {
        subtext: 'order',
        keys: ['orderPage', 'orderRowsPerPage', 'startDate', 'endDate'],
      },
    ];
    ...
    mapRoutesKeys.forEach(({ subtext, keys }) => {
      keys.forEach((key) => {
        if (!currentPath.includes(subtext)) {
          if (
            !localStorage.getItem('routeByOrder') ||
            !localStorage.getItem('routeByAnalytics') ||
            !localStorage.getItem('routeByShipment') ||
            !localStorage.getItem('routeByShipmentBySku')
          ) {
