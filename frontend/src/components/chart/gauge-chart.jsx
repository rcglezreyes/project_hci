    ...
    tickValues.forEach((value) => {
      ...
      ticks.push(
        <g key={value}>
          ...
          <text x={labelX} y={labelY} textAnchor="middle" fontSize="14" fill="#000">
            {value}
          </text>
        </g>
      );
  ...
  return (
    ...
    >
      <svg width="100%" height={size + 30} style={{ marginLeft: -6, marginTop: -4 }}>
        ...
        <text x={center} y={center + 40} textAnchor="middle" fontSize="24" fontWeight="bold">
          {`${normalizedPercentage}%`}
        </text>
        ...
        >
          {normalizedPercentage >= 80 ? 'High' : normalizedPercentage >= 50 ? 'Weak' : 'Bad'}
        </text>
      </svg>
    </Box>
  );
