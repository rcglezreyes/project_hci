...
import { MedicalStaffQuickEditForm } from './medical-staff-quick-edit-form';

// ----------------------------------------------------------------------

export function MedicalStaffTableRow({ row, selected, onSelectRow, onDeleteRow, refetchMedicalStaffs }) {
    ...
    return (
        <>
            {!isMobile ? (
                <TableRow hover selected={selected} aria-checked={selected} tabIndex={-1}>
                    ...
                    <TableCell sx={{ whiteSpace: 'nowrap', cursor: 'pointer' }} onClick={quickEdit.onTrue}>
                        <Stack sx={{ typography: 'body2', flex: '1 1 auto', alignItems: 'flex-start' }}>
                            <Box component="span" sx={{ color: 'text.disabled' }} key={`specialty-${row.id}`}>
                                {row.specialty.name}
                            </Box>
                        </Stack>
                    </TableCell>
                    ...
                    <TableCell sx={{ whiteSpace: 'nowrap', cursor: 'pointer' }} onClick={quickEdit.onTrue}>
                        <Stack sx={{ typography: 'body2', flex: '1 1 auto', alignItems: 'flex-start' }}>
                            <Box component="span" sx={{ color: 'text.disabled' }} key={`department-${row.id}`}>
                                {row.department.name}
                            </Box>
                        </Stack>
                    </TableCell>
                    ...
                    <TableCell sx={{ whiteSpace: 'nowrap', cursor: 'pointer' }} onClick={quickEdit.onTrue}>
                        {fDate(row.graduatedDate)}
                    </TableCell>

                    <TableCell sx={{ whiteSpace: 'nowrap', cursor: 'pointer' }} onClick={quickEdit.onTrue}>
                        {fDuration(row.graduatedDate, dayjs(new Date()))}
                    </TableCell>
                ...
                <TableRow hover selected={selected} aria-checked={selected} tabIndex={-1}>
                    ...
                    <TableCell sx={{ cursor: 'pointer' }}>
                        ...
                        <br />
                        Graduated Date: {fDate(row.graduatedDate)}
                        <br />
                        Specialty:
                        <Stack sx={{ typography: 'body2', flex: '1 1 auto', alignItems: 'flex-start' }}>
                            <Box component="span" sx={{ color: 'text.disabled' }} key={`specialty-${row.id}`}>
            ...
            <MedicalStaffQuickEditForm
                currentMedicalStaff={row}
                open={quickEdit.value}
                onClose={quickEdit.onFalse}
                refetchMedicalStaffs={refetchMedicalStaffs}
            />
