...
import { MedicalStaffTableRow } from '../medical-staff-table-row';
import { MedicalStaffTableToolbar } from '../medical-staff-table-toolbar';
import { MedicalStaffTableFiltersResult } from '../medical-staff-table-filters-result';
...
const TABLE_HEAD_MOBILE = [{ id: 'info', label: 'Medical Staff' }];

// ----------------------------------------------------------------------

export function MedicalStaffListView() {
  ...
  const { loadedMedicalStaffs, refetchMedicalStaffs } = useDataContext();
  ...
  useEffect(() => {
    const socket = new WebSocket(paths.backend.websocket.main.medicalStaff);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      ...
      const response = await axios.delete(paths.backend.api.main.medicalStaff.delete(id), {
        headers: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const response = await axios.delete(paths.backend.api.main.medicalStaff.deleteAll, {
        headers: {
          'Content-Type': 'application/json',
        },
        data: payload,
  ...
  }, [dataFiltered.length, dataInPage.length, table, tableData, userLogged, refetchMedicalStaffs]);
    ...
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          action={
            <Tooltip title="Create new medical staff">
              <Button
                component={RouterLink}
                href={paths.dashboard.medicalStaff.new}
                variant="outlined"
                // variant="contained"
              ...
              >
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box
                    component="img"
                    src={`${CONFIG.assetsDir}/assets/icons/notification/ic-create-medical-staff.svg`}
                    alt="patient"
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <MedicalStaffTableRow
                        ...
                        refetchPatients={refetchMedicalStaffs}
                      />
                    ))}
