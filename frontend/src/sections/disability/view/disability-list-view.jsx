...
import { DisabilityTableRow } from '../disability-table-row';
import { DisabilityTableToolbar } from '../disability-table-toolbar';
import { DisabilityTableFiltersResult } from '../disability-table-filters-result';

// ----------------------------------------------------------------------

const STATUS_OPTIONS = [{ value: 'all', label: 'All Disabilities' }].concat([
...
export function DisabilityListView() {
  const { isMobile } = useContext(LoadingContext);

  const userLogged = useMemo(() => JSON.parse(sessionStorage.getItem('userLogged')), []);

  const { loadedDisabilities, loadingDisabilities, errorDisabilities, refetchDisabilities } =
    useDataContext();
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('currentDisabilityId');
  }, []);
  ...
  useEffect(() => {
    if (refetchDisabilities) {
      refetchDisabilities();
    }
    setTableData(loadedDisabilities || []);
  }, [refetchDisabilities, loadedDisabilities]);

  useEffect(() => {
    if (loadedDisabilities) {
      setTableData(loadedDisabilities);
    }
  }, [loadedDisabilities]);

  useEffect(() => {
    const socket = new WebSocket(paths.backend.websocket.disability);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        const resp = await axios.delete(paths.backend.api.main.disability.delete(id), {
          data: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const resp = await axios.delete(paths.backend.api.main.disability.deleteAll, {
        data: {
          ids: table.selected,
          userReporter: JSON.stringify(userLogged?.data),
        },
  ...
  const handleEditRow = useCallback(
    (id) => {
      localStorage.setItem('currentDisabilityId', id);
      router.push(paths.dashboard.disability.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.disability.details(id));
    },
    [router]
  );

  if (loadingDisabilities) {
    return (
      <DashboardContent>
  ...
  if (errorDisabilities) {
    return (
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorDisabilities.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Disability', href: paths.dashboard.disability.root },
            { name: 'List' },
          ]}
          action={
            <Tooltip title="Create new disability">
              <Button
              ...
              >
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 0.2 }}>
                  ...
                  <Box
                    component="img"
                    src={`${CONFIG.assetsDir}/assets/icons/notification/ic-create-disability.svg`}
                    alt="disability"
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> disabilities?
          </>
        }
        action={
          <Button
