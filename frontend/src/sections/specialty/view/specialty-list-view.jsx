...
import { SpecialtyTableFiltersResult } from '../specialty-table-filters-result';

// ----------------------------------------------------------------------

const STATUS_OPTIONS = [{ value: 'all', label: 'All Specialtys' }].concat([
...
export function SpecialtyListView() {
  ...
  const { loadedSpecialties, loadingSpecialties, errorSpecialties, refetchSpecialties } =
    useDataContext();
  ...
  useEffect(() => {
    ...
    localStorage.removeItem('currentSpecialtyId');
  }, []);
  ...
  useEffect(() => {
    if (refetchSpecialties) {
      refetchSpecialties();
    }
    setTableData(loadedSpecialties || []);
  }, [refetchSpecialties, loadedSpecialties]);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        const resp = await axios.delete(paths.backend.api.main.specialty.delete(id), {
          data: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const resp = await axios.delete(paths.backend.api.main.specialty.deleteAll, {
        data: {
          ids: table.selected,
          userReporter: JSON.stringify(userLogged?.data),
        },
  ...
  const handleEditRow = useCallback(
    (id) => {
      localStorage.setItem('currentSpecialtyId', id);
      router.push(paths.dashboard.specialty.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.specialty.details(id));
    },
    [router]
  );
      ...
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorSpecialties.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Specialty', href: paths.dashboard.specialty.root },
            { name: 'List' },
          ]}
          action={
            <Tooltip title="Create new specialty">
              <Button
                component={RouterLink}
                href={paths.dashboard.specialty.new}
                // color="inherit"
                variant="outlined"
                // variant="contained"
              ...
              >
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 0.5 }}>
                  ...
                  <Box
                    component="img"
                    src={`${CONFIG.assetsDir}/assets/icons/notification/ic-create-specialty.svg`}
                    alt="specialty"
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> specialties?
          </>
        }
        action={
          <Button
