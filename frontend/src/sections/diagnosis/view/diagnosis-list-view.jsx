...
import { DiagnosisTableToolbar } from '../diagnosis-table-toolbar';
import { DiagnosisTableFiltersResult } from '../diagnosis-table-filters-result';

// ----------------------------------------------------------------------

const STATUS_OPTIONS = [{ value: 'all', label: 'All Diagnoses' }].concat([
...
export function DiagnosisListView() {
  ...
  const { loadedDiagnoses, loadingDiagnoses, errorDiagnoses, refetchDiagnoses } = useDataContext();
  ...
  useEffect(() => {
    ...
    setTableData(loadedDiagnoses || []);
  }, [refetchDiagnoses, loadedDiagnoses]);
  ...
  useEffect(() => {
    const socket = new WebSocket(paths.backend.websocket.main.diagnoses);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        const resp = await axios.delete(paths.backend.api.main.diagnosis.delete(id), {
          data: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const resp = await axios.delete(paths.backend.api.main.diagnosis.deleteAll, {
        data: {
          ids: table.selected,
          userReporter: JSON.stringify(userLogged?.data),
        },
  ...
  const handleEditRow = useCallback(
    (id) => {
      localStorage.setItem('currentDiagnosisId', id);
      router.push(paths.dashboard.diagnosis.edit(id));
    },
    [router]
  );
  ...
  const handleViewRow = useCallback(
    (id) => {
      router.push(paths.dashboard.diagnosis.details(id));
    },
    [router]
  );
      ...
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorDiagnoses.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Diagnosis', href: paths.dashboard.diagnosis.root },
            { name: 'List' },
          ]}
          action={
            <Tooltip title="Create new diagnosis">
              <Button
                ...
                href={paths.dashboard.diagnosis.new}
                // color="inherit"
                variant="outlined"
                // variant="contained"
              ...
              >
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box
                    component="img"
                    src={`${CONFIG.assetsDir}/assets/icons/notification/ic-create-diagnosis.svg`}
                    alt="diagnosis"
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> diagnoses?
          </>
        }
        action={
          <Button
