...
import { RoomTableFiltersResult } from '../room-table-filters-result';

// ----------------------------------------------------------------------

const STATUS_OPTIONS = [{ value: 'all', label: 'All Rooms' }].concat([
...
export function RoomListView() {
  ...
  const { loadedRooms, loadingRooms, errorRooms, refetchRooms } = useDataContext();
  ...
  useEffect(() => {
    ...
    setTableData(loadedRooms || []);
  }, [refetchRooms, loadedRooms]);
  ...
  useEffect(() => {
    const socket = new WebSocket(paths.backend.websocket.main.rooms);
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      try {
        const resp = await axios.delete(paths.backend.api.main.room.delete(id), {
          data: {
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      const resp = await axios.delete(paths.backend.api.main.room.deleteAll, {
        data: {
          ids: table.selected,
          userReporter: JSON.stringify(userLogged?.data),
        },
      ...
      <DashboardContent>
        <Box display="flex" alignItems="center" mb={5}>
          <Alert severity="error" sx={{ borderRadius: 0 }}>
            <Typography>Error fetching items: {errorRooms.message}</Typography>
          </Alert>
        </Box>
      </DashboardContent>
    );
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Room', href: paths.dashboard.room.root },
            { name: 'List' },
          ]}
          action={
            <Tooltip title="Create new room">
              ...
              >
                <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box
                    component="img"
                    src={`${CONFIG.assetsDir}/assets/icons/notification/ic-create-room.svg`}
                    alt="room"
      ...
      <ConfirmDialog
        ...
        content={
          <>
            Are you sure want to delete <strong> {table.selected.length} </strong> rooms?
          </>
        }
        action={
          <Button
